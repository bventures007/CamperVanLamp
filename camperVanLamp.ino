 ...
 */
#include "PinDefinitonsAndMore.h"
#define WHITE 5
#define YELLOW 6

#define PWMSTEPS 50

int pwmSignal=0,savePwmDuringOff=100;
unsigned int pwmChannel=YELLOW;
...
void setup() {
    ...
    pinMode(WHITE, OUTPUT);  // sets the pin as output
...
void loop() {
    ...
    if (irmp_get_data(&irmp_data)) {
        ...
        if (!(irmp_data.flags & IRMP_FLAG_REPETITION)) {
            ...
            case 0x11:
               // Serial.println("B");
                pwmSignal = pwmSignal-PWMSTEPS;
                if(pwmSignal<0)pwmSignal=0;
                break;
            case 0xC:
                //Serial.println("L");
                pwmChannel=WHITE; 
            ...
            case 0x2:
                //Serial.println("O");
                if(pwmSignal==0)
                  pwmSignal=savePwmDuringOff;
                else {
                   savePwmDuringOff=pwmSignal;
                   pwmSignal=0;
                }
                break;
            ...
            analogWrite(pwmChannel, pwmSignal);
        }
